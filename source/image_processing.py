...
def get_contour_path(image: np.ndarray) -> list:
    ...
    closest_countours = [inf, 0] # used for reordering contours (choose first-last contours to be part of shortest contour pair)
    ...
    while len(filtered_contours) > 0:
        ...
        if shortest[0] < closest_countours[0]:
            closest_countours = [shortest[0], len(ordered_contours) - 1]

    ordered_contours = ordered_contours[closest_countours[1:] + ordered_contours[:closest_countours[1]]]

    ordered_contour = reduce(lambda a,b: np.concatenate((a, b)), ordered_contours)

    return ordered_contour

def seperate_coords(contour_path: list) -> tuple:
...
def get_time_domain_func(image: np.ndarray) -> tuple:
    ...
    f_time_x, f_time_y = seperate_coords(contour_path)
